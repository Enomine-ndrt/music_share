...
export const UPDATE_ARTIST_BANNER = 'UPDATE_ARTIST_BANNER';
...
/**
 * Modified banner and banner
 */
export const updateArtistBA = (banner, avatar, color, id_artista) => {
    try {

        return async dispatch => {
            const API_URL = BASE_URL + `music_share/php-rest-api/api/artista/update.php`;
            const results = await axios.post(API_URL, {
                "banner": banner,
                "avatar": avatar,
                "colorBanner": color,
            ...
            console.log('response ', results);
            await dispatch({
                type: UPDATE_ARTIST_BANNER,
                payload: results
            });

        }
    } catch (error) {
        console.log('Ha ocurrido un error al modificar banner and avatar ', error.message);
    }
}
